...
namespace menon
{
  ...
  /// @param[in]  c     探索する文字
  /// @return     文字が見つかればその文字へのポインタを、見つからなければnullptrを返す。
  /// 標準関数のstrrchrと同じだが、constexprにできる点が異なる。
  template <typename Char>
  constexpr auto strrchr(Char* s, std::remove_cv_t<Char> c)
    -> Char*
  {
    if (s == nullptr)
      return nullptr;
    ...
    auto t = sv(s);
    using sv_type = decltype(t);
    auto pos = t.rfind(static_cast<typename sv_type::value_type>(c));
    if (pos == sv_type::npos)
      return nullptr;
    return s + pos;
  }
  ...
  /// @param[in]  c     探索する文字
  /// @return     文字が見つかればその文字への反復子を、見つからなければ空の反復子を返す。
  /// 標準関数のstrrchrと同じだが、stringやstring_viewに対しても使用できる。
  template <typename String, typename Char>
  constexpr auto strrchr(String const& s, Char c)
    -> decltype(std::begin(s))
  {
    using ::menon::sv;
    auto t = sv(s);
